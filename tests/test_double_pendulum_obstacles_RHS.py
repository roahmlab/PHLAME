...
class TestAghfActivatedObstacleContraintsDoublePendulum(unittest.TestCase):
    def setUp(self):
        base_dir = os.path.join(os.path.dirname(__file__), "..")
        fp_data_aghf_dp = os.path.join(base_dir, "tests/matlab_files/data_aghf_activated_obs_constraint.mat")
        ...
        self.single_obs_mat = self.data['single_obs_mat'].astype(dtype=np.double, order='F')
    
    def test_update_AGHF_with_obstacle_constraints(self):
        print("####### Running test_double_pendulum_obstacles_RHS")
        ...
        for ii in range(self.num_tests):
            ...
            obstacles = self.single_obs_mat[:, ii].reshape(1, -1)

            aghf_wrapper = aghf_pybind.AghfPybindWrapper(self.fp_urdf, k, self.j_type)
            aghf_wrapper.set_activated_obstacles(obstacles, c_cons, k_cons)
